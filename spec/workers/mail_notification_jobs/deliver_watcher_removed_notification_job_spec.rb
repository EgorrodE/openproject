...
describe DeliverWatcherRemovedNotificationJob, type: :model do
  ...
  let(:watcher_remover) { FactoryBot.create(:user) }
  let(:watcher_user) do
  ...
  subject { described_class.new.perform(watcher.attributes, watcher_user.id, watcher_remover.id) }

  before do
    # make sure no actual calls make it into the UserMailer
    allow(UserMailer).to receive(:work_package_watcher_removed)
      .and_return(double('mail', deliver_now: nil))
  end

  it 'sends a mail' do
    expect(UserMailer).to receive(:work_package_watcher_removed).with(work_package,
                                                                    watcher_user,
                                                                    watcher_remover)
    subject
  end
  ...
  describe 'exceptions' do
    describe 'exceptions should be raised' do
      before do
        ...
        expect(UserMailer).to receive(:work_package_watcher_removed).and_return(mail)
      end
